...
const getExpectedWeights = (baseWeightsNumbers: number[], fixWeightsNumbers: number[]): BigNumber[] => {
  ...
  const sign = totalDenormalizedBaseWeight.add(totalDenormalizedFixedWeight).gt(HUNDRED_PERCENT) ? -1 : 1;
  ...
  const finalWeights = baseWeights.map((initialBaseWeight: BigNumber, idx: number) => {
    ...
    const adjustedBaseWeight = sign < 0 ? initialBaseWeight.add((initialBaseWeight / totalDenormalizedBaseWeight) * delta)
...
describe('IndexPoolUtils', function () {
  ...
  describe('#normalizeInterpolated', () => {
    describe('with denormalized weights greater than one', () => {
      ...
      //   it.only('returns the correct interpolated normalized weights', async () => {
      //     const expectedWeights = getExpectedWeights(baseWeights, fixedWeights);
      //     const receivedWeights = await normalizerInstance.normalizeInterpolated(
      //       baseWeights.map((w) => toBnPercentage(w)),
      //       fixedWeights.map((w) => toBnPercentage(w))
